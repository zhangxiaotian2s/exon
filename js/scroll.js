		...
		function  MultipleBtnfn(){
			...
			rightBtn.click(function() {
				...
				if (thisi >= Math.ceil(ImgBox.children(Parameter.ImgBoxChild).length/Parameter.BnanerGeshu)) {
					...
					var thisi=ImgBox.children(Parameter.ImgBoxChild).filter(":visible").index()+1;
			 
			 	if(thisi > ImgBox.children(Parameter.ImgBoxChild).length-Parameter.deomShownum){
			...
			//显示部分的长度
				...
				if (thisi < 0) {
			 	...
			 	if(thisi < 0){
			 		...
			 		thisi=ImgBox.children(Parameter.ImgBoxChild).length-Parameter.deomShownum
			 	}
